...
<div id="central-ID" class="central">
  ...
  /***********************************************************************************/
         ...
         // REMOVER DOCUMENTOS  //
        const banerList = document.querySelector('[data-js="container-baner"]')
        db.collection('baner').get()
        .then(snapshot => {
            const banerLis = snapshot.docs.reduce((acc, doc) => {
...
<div id="central-ID" class="central">
    <div id="container-pedidos-ID" class="container">
        <div id="card-ID" class="card">
            ...
            banerList.innerHTML = banerLis
        })
        .catch(err => {
            console.log(err.message)
    })

        banerList.addEventListener('click', e =>{
            const removeButtonId = e.target.dataset.remove

             if (removeButtonId){

                db.collection('baner').doc(removeButtonId).delete()
                .then(() =>{

                alert('baner removido com sucesso!')

                 const baner = document.querySelector(`[data-id="${removeButtonId}"]`)
              baner.remove()

                    console.log('baner removido com sucesso!')
                })
                .catch(e => {
                    console.log('Erro ao remover a baner', e.message)
                })
...
const formCliente00 = document.querySelector('#form-cliente00');
        formCliente00.addEventListener('submit', (e) => {
            e.preventDefault();
            
            let arquivo = document.querySelector('[name=arquivo00]').files[0];

            const uploadTask00= storage.ref('baner/' + arquivo).put(arquivo);

            uploadTask00.on('state_changed', (snapshot) => {
                const progress00 = (snapshot.bytesTransferred / snapshot.totalBytes) * 1;
                console.log(progress00);
            },
                function (error) {
